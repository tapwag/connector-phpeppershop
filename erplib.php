...
class erp {
    ...
    function getTax() {
        ...
        if ($rs) foreach ($rs as $row) {
            ...
            if ( !array_key_exists($nr, $this->TAX) ) {
                $data = array();
    ...
    function insParts($trans_id,$data,$longtxt) {
        ...
        foreach ($data as $row) {
             ...
             // brauche ich die pricegroup_id wirklich?
             $sql  = "INSERT INTO orderitems (trans_id,parts_id,description,longdescription,qty,sellprice,unit,discount) ";
             $sql .= "VALUES (:trans_id,:parts_id,:description,:longdescription,:qty,:sellprice,:unit,0)";
             $row["trans_id"]=$trans_id;
    ...
    function insCustomer($data) {
        ...
        if ($data['customernumber']>0) {
            ...
            $sql  = "INSERT INTO customer (greeting,name,street,city,zipcode,country,contact,phone,email,customernumber,currency_id)";
            $sql .= " VALUES (:greeting,:name,:street,:city,:zipcode,:country,:contact,:phone,:email,:customernumber,:currency_id)";
            $rc =  $this->db->insert($sql,$data);
    ...
    function mkAuftrag($data,$shop,$longtxt) {
        ...
        echo "!!".$data['curr']."!".$data['currency_id']."!<br>";
    ...
    function showDiff($shop,$erp) {
        ...
        if ( $diff != '' ) {
            echo '<tr><td>'.$shop['partnumber'].'</td><td>'.substr($shop['description'],0,40).'</td><td>'.$diff.'</td></tr>';
        }
    }
