...
class pepper {
    ...
    var $custshop   = array("kontakt"=>"contact","ort"=>"city","plz"=>"zipcode","land"=>"country","tel"=>"phone",
                            ...
                            "iban"=>"iban","bic"=>"bic","attributwert1"=>"ustid","kunden_nr"=>"kunden_nr");
    ...
    function pepper($db,$error,$dbname,
                    ...
                    $mwstLX,$mwstPGLX,$pricegroup,$mwstS,$variantnr,$pic=false,$nopic=false,$nopicerr=false,$nofiles=false,
    ...
    function getAllArtikel($Varianten=False) {
        $f = fopen("/tmp/tr","w");
        fputs($f,$this->shopcode." : ".$this->erpcode."\n");
        if (!$this->Kategorien) $this->getKategorien();
        if ($Varianten) {
            ...
            $sql .= "FROM artikel a ";
            //$sql .= "LEFT JOIN artikel_kategorie k ON a.artikel_id = k.fk_artikel_id ";
            ...
            $sql .= "FROM artikel a ";//LEFT JOIN artikel_kategorie k on a.artikel_id = k.fk_artikel_id";
        }
        $rs = $this->db->getAll($sql);
        if ($rs) foreach ($rs as $row) {
            ...
            fputs($f,$row['nr']." : ".$row['anzahl_einheit']." ".$row['name']."\n");
            $data[] = $this->translateTable($row,"orderparts");
        }
        fclose($f);
        return $data;
